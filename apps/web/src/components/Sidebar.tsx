      ...
      <nav className="nav-links">
        ...
        <div className="nav-section">
          ...
          {products.map((product) => (
            <SidebarLink key={product.id} href={`/products/${product.id}`} className="nav-link nav-link-sub">
              {product.name}
            </SidebarLink>
          ))}
          {products.length === 0 ? <span className="nav-link-empty">Нет продуктов</span> : null}
        </div>

        <div className="nav-section">
          <span className="nav-section-title">Система</span>
          <SidebarLink href="/archive" className="nav-link">
            {"\u0410\u0440\u0445\u0438\u0432"}
          </SidebarLink>
